                    ...
                    if let dayEntry = dayEntry,
                       let info = self.parseDateInfo(from: dayEntry.dt_txt) {
                        forecastDay.append(Forecast(
                            ...
                            condition: dayEntry.weather.first?.main,
                            conditionId: dayEntry.weather.first?.id,
                            symbolName: self.sfSymbolName(for: dayEntry.weather.first?.id, isNight: false)
                        ))
                    }
                    ...
                    if let nightEntry = nightEntry,
                       let info = self.parseDateInfo(from: nightEntry.dt_txt) {
                        forecastNight.append(Forecast(
                            ...
                            condition: nightEntry.weather.first?.main,
                            conditionId: dayEntry.weather.first?.id,
                            symbolName: self.sfSymbolName(for: nightEntry.weather.first?.id, isNight: true)
                        ))
                    }
                }
